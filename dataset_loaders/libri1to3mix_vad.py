...
LIBRI3MIX_ROOT_PATH = '/home4/dataset/LibriMix/Libri3Mix'   #60666  #  '/mnt/data/libri_mix/Libri3Mix'
# LIBRI3MIX_ROOT_PATH = '/home/YangQ/Datasets/LibriMix/Libri3Mix'   #A40
...
class Dataset(torch.utils.data.Dataset, abstract_dataset.Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        self.zero_pad = self.get_arg_and_check_validness(  #check and validness
        ...
        self.min_or_max = self.get_arg_and_check_validness(
            'min_or_max', known_type=str, choices=['min', 'max'])  #min
        ...
        self.n_samples = self.get_arg_and_check_validness(
            'n_samples', known_type=int, extra_lambda_checks=[lambda x: x >= -1]) #use lambda function to chack
        ...
        self.sample_rate = self.get_arg_and_check_validness(
            'sample_rate', known_type=int, choices=[8000, 16000])  #16000

        self.n_speakers_cdf = np.cumsum(self.n_speakers_priors).tolist()  # n_speakers_cdf=[0.5,0.75,1.0]

        self.dataset_dirpath = self.get_path()   #home4/dataset/librimix/libri3mix/wave16k/min/[split]

        self.available_filenames = [
            os.path.basename(f) for f in         #path：home4/dataset/librimix/libri3mix/wave16k/min/train-360/s1/…….*wave
            glob2.glob(os.path.join(self.dataset_dirpath, 's1') + '/*.wav')]  #…….*wave
        ...
        if self.split == 'train-360':   ###add
            # self.n_samples = len(self.available_filenames)   ###add##################################################
            self.n_samples = 23325//3  ## the number of chime dataset (use vad)
            random.shuffle(self.available_filenames)
            self.available_filenames = self.available_filenames[:self.n_samples]
        elif self.n_samples <= 0:   #n_samples=-1
            self.n_samples = len(self.available_filenames)
        # if self.n_samples <= 0:   #n_samples=-1
        #     self.n_samples = len(self.available_filenames)
        else:
            self.available_filenames = self.available_filenames[:self.n_samples]
        ...
        for idx, fname in enumerate(sorted(self.available_filenames)):
            this_n_sources = 3
            for j in range(3):
                if idx / self.n_samples <= self.n_speakers_cdf[j]:   #n_speakers_cdf=[0.5,0.75,1.0]
                    this_n_sources = j + 1                           #train(librimix-train_360，共33900条): idx<=16950时j=0，说话人就j+1=1个，16950<idx<=25425时候j=1，说话人2个，idx>25425时j=2，说话人3个
                    break                                            #dev(librimix-dev,共3000条): idx<=1500时说话人1个，1020<idx<=2250时说话人2个，idx>2250时说话人3个
                                                                     #test(librimix-test,共3000条): idx<=1500时说话人1个，1020<idx<=2250时说话人2个，idx>2250时说话人3个

            if self.fixed_n_sources < 0 or (this_n_sources == self.fixed_n_sources):  #fixed_n_sources=1/2/3
    ...
    def get_path(self):
        path = os.path.join(LIBRI3MIX_ROOT_PATH,
                            'wav{}k'.format(int(self.sample_rate / 1000)), #sample_rate = 16000
                            self.min_or_max, self.split)  #LIBRI3MIX_ROOT_PATH/wav16k/min/split
    ...
    def __len__(self):  # return length of the datasets
        return self.n_samples

    def __getitem__(self, idx):  #Returns the elements in the data set based on the index value
       ...
       #####speech
        ...
        sources_tensor = torch.stack(src_tensor_list, 0)   #[3,time_length*sample_rate]
...
def test_generator():
    ...
    for sources, noise in generator:
        ...
        break
    this_time = time.time() - before    #计算执行上列代码所花费的时间
