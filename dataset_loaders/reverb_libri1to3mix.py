    ...
    def __getitem__(self, idx):
        ...
        for src_idx in range(3):
            if src_idx + 1 <= file_info['n_active_sources']:
                src_w_ori = self.wavread(file_info['sources_paths'][src_idx])
                src_w = self.simulate_a_source_in_a_room(src_w_ori.numpy(), room_params)
                src_w = src_w[:max_len]
                source_tensor = self.get_padded_tensor(src_w, start_index=start_index)
                source_tensor_ori = self.get_padded_tensor(src_w_ori, start_index=start_index)
            else:
                source_tensor = torch.zeros_like(noise_tensor)
                source_tensor_ori = torch.zeros_like(noise_tensor)
            src_tensor_list.append(source_tensor)
            src_ori_tensor_list.append(source_tensor_ori)
        sources_tensor = torch.stack(src_tensor_list, 0)
        source_tensor_ori = torch.stack(src_ori_tensor_list,0)

        return sources_tensor, noise_tensor.unsqueeze(0),source_tensor_ori
