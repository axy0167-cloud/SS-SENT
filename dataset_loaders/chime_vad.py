...
CHiME_ROOT_PATH = '/home4/dataset/CHiME_audio_segement/'   ##60666      #  '/mnt/data/CHIME/processed_CHiME/'
# CHiME_ROOT_PATH ='/home/YangQ/Datasets/CHiME_audio_segement'  ###A40
...
class Dataset(torch.utils.data.Dataset, abstract_dataset.Dataset):
    ...
    def __init__(self, **kwargs):
        ...
        if self.split == 'train' and not self.use_vad:
            self.available_filenames = [
                os.path.join(os.path.split(os.path.split(f)[0])[1], os.path.basename(f))  #unlabeled_10s/*.wav
                for f in glob2.glob(self.dataset_dirpath + '/unlabeled_10s/*.wav')]   #home4/dataset/CHiME_audio_segment/train/unlabeled_10s/*.wav
        ...
        elif self.get_only_active_speakers and self.fixed_n_sources < 0:   #train的sub_set只有unlabeled/unlabeled_10s/unlabeled_vad/unlabeled_vad_10s
            self.available_filenames = []
            for i in range(1, 4):
                self.available_filenames += \
                    [os.path.join(os.path.split(os.path.split(f)[0])[1], os.path.basename(f))
          ...
          #val: sub_set: home4/dataset/CHiME_audio_segment/dev/1/*.wav
        else:
            self.available_filenames = [
                os.path.join(os.path.split(os.path.split(f)[0])[1], os.path.basename(f))  #1/*.wav
        ...
        # if self.split == 'train':
        #     self.n_samples = 33900           ##############################################################################################################
        #     self.available_filenames = self.available_filenames[:self.n_samples]
        if self.n_samples <= 0:  #train：-1   val：250
            self.n_samples = len(self.available_filenames)//3
            self.available_filenames = self.available_filenames[:self.n_samples]
        else:
            self.available_filenames = self.available_filenames[:self.n_samples]
    ...
    def __getitem__(self, idx):
        ...
        return mixture_tensor   ##############################################添加了unsqueeze(0)
