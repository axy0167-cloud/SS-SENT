...
class Dataset:
    ...
    def safe_pad(self, tensor_wav, dtype=torch.float32):
        if self.zero_pad and tensor_wav.shape[0] < self.time_samples:
            appropriate_shape = tensor_wav.shape   #tensor_wav.shape(128000,)
            padded_wav = torch.zeros(         #padded_wav = torch.zeros([self.time_samples], dtype=dtype)
    ...
    def get_arg_and_check_validness(self,
                                    key,
                                    choices=None,   #choices=[8000, 16000]
                                    known_type=None,  #int
