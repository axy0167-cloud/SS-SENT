...
from utils.utils import has_arg
# from baseline.utils.utils import has_arg
...
def pairwise_neg_sisdr(source, est_source, scale_invariant=True):
    ...
    """
    # if used complex spectrum

    # stft = STFT()
    # istft = ISTFT()
    # source_s = stft(source)
    # source = istft(source_s)
...
if __name__ == '__main__':
    ...
    feat_q = torch.randn(B, dim)
    feat_p = torch.randn(B, dim)
    feat_n = torch.randn(2 * B, dim)

    contrastive_loss = PatchNCELoss(B)
    loss = contrastive_loss(feat_q, feat_p, feat_n)
    print(loss)
